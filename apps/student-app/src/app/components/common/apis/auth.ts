import {
  Permission,
  Role,
} from '../../../pages/roles-management/RoleManagement';
...
export async function createRoleAPI(
  role: Omit<Role, 'id' | 'createdAt' | 'permissions'> & {
    permissions?: number[];
  }
) {
...
export async function assignPermissionsToRoleAPI(
  ...
  if (!response.ok) {
    const errorData = await response.json();
    throw new Error(
      errorData.message || 'Failed to assign permissions to role'
    );
  }
  const data = await response.json();
...
export async function getPermissionsAPI(): Promise<Permission[]> {
  ...
  const response = await fetch(
    `${API_URL}/v1/permissions?pageNo=1&pageSize=100`,
    {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
